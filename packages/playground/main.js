...
+  if (!watLSP) {
+    console.warn('registerFoldingRangeProvider: watLSP not available');
+    return;
+  }
+
+  try {
+    monaco.languages.registerFoldingRangeProvider('wat', {
...
+        watLSP.parse(model.getValue());
+
+        // Get folding ranges from the LSP
+        const ranges = watLSP.provideFoldingRanges();
+
+        // Convert to Monaco folding ranges
+        return ranges.map(range => ({
+          start: range.startLine + 1,  // Monaco is 1-indexed
+          end: range.endLine + 1,
...
+    });
+
+    consoleOutput.info('Folding range provider registered');
+  } catch (e) {
+    console.error('Failed to register folding range provider:', e);
+    consoleOutput.error('Failed to register folding ranges: ' + e.message);
+  }
+}
+
 // Initialize LSP
